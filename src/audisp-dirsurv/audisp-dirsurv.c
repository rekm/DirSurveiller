 ...
 *
 * Authors:
 *   Rene Kmiecinski <r.w.kmeicinski@gmail.com>
 * Based on audit_remote by:
...
static void process_one(struct queue *queue, auparse_state_t *au)
{
    char event[MAX_AUDIT_MESSAGE_LENGTH];
    int len;

    if (suspend || !transport_ok){
        syslog(LOG_NOTICE, "Cannot process event: suspend or bad transport");
        return;
    }
    //syslog(LOG_NOTICE, "Doing stuff");
...
/**
 * Callback function used to preprocess and filter audit-events
 * Uses auparse to extract all relevant information from audit-event
 */
...
int main(int argc, char *argv[])
{
        ...
        if (FD_ISSET(ifd, &rfd)) {
            do {
                if (remote_fgets(event, sizeof(event), ifd)) {
                    ...
                    syslog(LOG_INFO, "remote fgets got EOF!");
                    stop = 1;
                }
    ...
    if (is_pipe(0)) {
        syslog(LOG_INFO, "Detected stdin is pipe");
        syslog(LOG_INFO, "Transport is%s ok", transport_ok ? "":" not");
        if (suspend) syslog(LOG_INFO, "Event processing is suspended");
        while (q_queue_length(queue) && !suspend &&
               //!stop &&
               transport_ok)
        {
...
static int init_sock(void)
{
    ...
    while (runp) {
        ...
        if (config.local_port != 0) {
            ...
            // bind socket or skip to next try {{{
...
// TODO: setup socket info {{{
    ...
    // Creating adresses
    if (snprintf(soc_addr.sun_path, sizeof(soc_addr.sun_path),
                 "%s", config.domain_socket) >= sizeof(soc_addr.sun_path))
    {
        syslog(LOG_ERR, "Can't set socket path: path too long");
        rc=ET_PERMANENT;
        goto out;
    }
...
// tcp send and recieve message handling {{{
